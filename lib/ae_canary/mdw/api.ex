defmodule AeCanary.Mdw.Api do
  ...
  def outgoing_spend_txs(sender_id, from, to) do
    old_locations =
      Transactions.list_locations_of_spend_txs_by(%{
        sender_id: sender_id,
        select: :location,
        from: from,
        to: to
    ...
    maybe_insert = fn new_batch -> Transactions.maybe_insert_list(new_batch, old_locations) end
    delete_old = fn -> Enum.each(old_locations, &Transactions.delete_tx_by_location/1) end

    paged_update_db(
      "txs/gen/#{to}-#{from}?spend.sender_id=#{sender_id}&limit=#{@limit}",
      &Tx.decode/1,
      maybe_insert,
      delete_old
    )
  ...
  def incoming_spend_txs(recipient_id) do
    old_locations =
      Transactions.list_locations_of_spend_txs_by(%{recipient_id: recipient_id, select: :location})

    maybe_insert = fn new_batch -> Transactions.maybe_insert_list(new_batch, old_locations) end
    delete_old = fn -> Enum.each(old_locations, &Transactions.delete_tx_by_location/1) end

    paged_update_db(
      "txs/backward?spend.recipient_id=#{recipient_id}&limit=#{@limit}",
      &Tx.decode/1,
      maybe_insert,
      delete_old
    )
  ...
  def incoming_spend_txs(recipient_id, from, to) do
    ...
    paged_update_db(
      "txs/gen/#{to}-#{from}?spend.recipient_id=#{recipient_id}&limit=#{@limit}",
      &Tx.decode/1,
      maybe_insert,
      delete_old
    )
  ...
  defp paged_update_db(uri, decode, maybe_insert, delete_old) do
    case get(uri) do
      ...
      {:ok, %{data: data, next: next_uri}} ->
        ...
        case res do
          ## all records are new, read next batch
          :inserted_all ->
            paged_update_db(next_uri, decode, maybe_insert, delete_old)

          ## we found previous tx, abort fetching more txs
          :ok ->
            :ok
        end
