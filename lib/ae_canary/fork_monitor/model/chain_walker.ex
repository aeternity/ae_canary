defmodule AeCanary.ForkMonitor.Model.ChainWalker do
  ...
  def updateChainEnds(max_depth, notify_pid \\ nil) do
    ...
    case Enum.member?(chainEndHashes, current_generation["key_block"]["hash"]) do
      true ->
        Logger.info("Inserting recent transactions from top #{current_generation["key_block"]["hash"]}")
        Model.TransactionWalker.update_spend_transactions_from_chain_end(current_generation["key_block"]["hash"])
      false ->
        Logger.info("Not inserting recent transactions because current generation #{current_generation["key_block"]["hash"]} not in known chain ends")
    end
